...
func (c *customChain) exists(prog iptablesInterface) (bool, error) {
	...
	if err != nil {
		...
		return false, fmt.Errorf("unable to list %s chain: %s (%s)", c.name, string(output), err)
	}
...
func (c *customChain) doAdd(prog iptablesInterface) error {
	...
	output, err := prog.runProgCombinedOutput(args)
	if err != nil {
		return fmt.Errorf("unable to add %s chain: %s (%s)", c.name, string(output), err)
	}
...
func (c *customChain) doRename(prog iptablesInterface, newName string) error {
	...
	output, err := prog.runProgCombinedOutput(args)
	if err != nil {
		return fmt.Errorf("unable to rename %s chain to %s: %s (%s)", c.name, newName, string(output), err)
	}
...
func (c *customChain) doRemove(prog iptablesInterface) error {
	...
	output, err := prog.runProgCombinedOutput(args)
	if err != nil {
		return fmt.Errorf("unable to flush %s chain: %s (%s)", c.name, string(output), err)
	}
	...
	output, err = prog.runProgCombinedOutput(args)
	if err != nil {
		return fmt.Errorf("unable to remove %s chain: %s (%s)", c.name, string(output), err)
	}
...
func (c *customChain) doInstallFeeder(prog iptablesInterface, feedArgs string) error {
	...
	if feedArgs != "" {
		...
		if err != nil {
			log.WithError(err).WithField(logfields.Object, feedArgs).Fatal("Unable to parse rule into argument slice")
		}
	...
	output, err := prog.runProgCombinedOutput(args)
	if err != nil {
		return fmt.Errorf("unable to install feeder rule for %s chain: %s (%s)", c.name, string(output), err)
	}
